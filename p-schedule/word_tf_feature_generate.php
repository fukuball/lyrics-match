...
foreach ($query_result as $query_result_data) {
   ...
   $select_sql = "SELECT ltu.*,IF(ltt.tfidf IS NULL, 0, ltt.tf) tf FROM lyrics_term_unique ltu LEFT JOIN lyrics_term_tfidf ltt ON (ltu.term=ltt.term AND ltt.song_id='$song_id')";
   ...
   foreach ($query_result2 as $query_result_data2) {
      $not_all_zero = $not_all_zero+$query_result_data2['tf'];
      $term_vector_string = $term_vector_string.$query_result_data2['tf'].',';
